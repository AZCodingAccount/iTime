...
在每个窗口webPreference配置这个选项，表示进入后台后拒绝让electron关闭定时器这种资源
...
1. 修复了定时器half提醒时重复提醒的问题
2. 修复一定条件下自定义代办挂件内容不显示的问题
3. 修复自定义设置图标回显问题

由于`electron-builder`不能在windows上打mac的包，暂时只提供三种windows安装包版本。程序占用大概**280MB**

- iTime-1.0.3-windows-installer.exe。常规的exe安装包，允许你自定义安装路径。
- iTime-1.0.3.msi。微软官方安装包，一键安装。程序相关文件会存储在`AppData\Local\Programs\iTime`下
- iTime-1.0.3-windows-no-installer.exe。exe文件，下载点击即可运行。程序相关文件会存储在`AppData\Local\Temp`下
